function [est,N,ctr,x,dx,coef]=centros(N,simul_Lu,simul_frontera,rbf,Lrbf,eps1,Nmax,tol1,tol2)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%Generación el conjunto inicial de centros y puntos de colocacion. 

v=linspace(0,1,sqrt(N));
nv=length(v);
dx=1/(sqrt(N)-1); %Separción entre puntos de la rejilla inicial.
%Puntos de colocación y centros del borde.
xb=[[zeros(1,nv);v],[ones(1,nv);v],[v(2:end-1);zeros(1,nv-2)],[v(2:end-1);ones(1,nv-2)]];
%Puntos de colocación y centros en la frontera. 
v(1)=[];
v(end)=[];
[X,Y]=meshgrid(v,v);
xi=[X(:)'; Y(:)'];
nv=length(v);
x=[xi xb]; %Matriz con todos los puntos de colocación
ctr=[xb xi]; % Matriz de centros
